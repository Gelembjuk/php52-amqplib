...
class PhpAmqpLib_Connection_AbstractConnection extends PhpAmqpLib_Channel_AbstractChannel
{
    ...
    /** @var PhpAmqpLib_Wire_AMQPWriter */
    ...
    /** @var PhpAmqpLib_Wire_IO_SocketIO */
    protected $sock;
    ...
    /** @var _PhpAmqpLib_Wire_AMQPReader */
    ...
    public function __construct(
        ...
        $this->wait_frame_reader = new PhpAmqpLib_Wire_AMQPReader(null);
        $this->vhost = $vhost;
        $this->insist = $insist;
        ...
        if ($user && $password) {
            $this->login_response = new PhpAmqpLib_Wire_AMQPWriter();
     ...
     * @param string $args
     * @param PhpAmqpLib_Wire_AMQPWriter $pkt
     * @return null|PhpAmqpLib_Wire_AMQPWriter
     */
    ...
    protected function wait_frame($timeout = 0)
    {
        ...
        try {
            ...
            $this->wait_frame_reader->reuse(
                $this->input->read(PhpAmqpLib_Wire_AMQPReader::OCTET + PhpAmqpLib_Wire_AMQPReader::SHORT + PhpAmqpLib_Wire_AMQPReader::LONG)
            );
            ...
            if ($frame_channel === 0 && $frame_type === 8) {
                ...
                if($_timeout > 0)
                {
                    ...
                    if($_timeout <= 0)
                    {
                        ...
                        throw new PhpAmqpLib_Exception_AMQPTimeoutException("Timeout waiting on channel");
                    }
                }
                continue;
     ...
     *
     * @param string $channel_id
     * @return PhpAmqpLib_Channel_AMQPChannel
     */
     ...
     * @throws PhpAmqpLib_Exception_AMQPProtocolConnectionException
     */
    ...
    /**
     * @return PhpAmqpLib_Wire_IO_AbstractIO
     */
     ...
     */
    protected function connection_unblocked(PhpAmqpLib_Wire_AMQPReader $args)
    {
